...
from video.serializers import (
    PlaylistSerializer,
    PlaylistVideoAddSerializer,
    VideoCategorySerializer,
    VideoPatchSerializer,
    VideoSerializer,
)
...
class VideoViewSet(BaseAuthModelViewSet):
    ...
    def get_serializer_class(self):
        serializer_mapping = {"PATCH": VideoPatchSerializer, "GET": VideoSerializer}
    ...
    def create(self, request, *args, **kwargs):
        ...
        if action == "add":
            pv = PlaylistVideo.objects.filter(
                playlist_id=data["playlist"], video_id=data["video"]
            )
            if pv:
                return Response(
                    {"error": "The video is already in playlist"},
            ...
            pvs = list(
                PlaylistVideo.objects.filter(playlist=data["playlist"]).values(
                    "video", "video__name"
                )
            )
            videos = []
            for pv in pvs:
                videos.append({"id": pv["video"], "name": pv["video__name"]})
